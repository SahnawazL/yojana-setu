    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",overflowY:"auto"}}>
      ...
      <div style={{background:"#fff",padding:"16px 16px 0",position:"sticky",top:0,zIndex:10,borderBottom:"1px solid #f0f0f0"}}>
        ...
        <div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:12}}>
          <div onClick={()=>setFilter("all")} style={{flexShrink:0,padding:"6px 14px",borderRadius:20,fontSize:12,fontWeight:700,background:filter==="all"?"#003580":"#f5f5f0",color:filter==="all"?"#fff":"#555",cursor:"pointer",border:`1.5px solid ${filter==="all"?"#003580":"#e0e0e0"}`}}>
            {isHindi?"सभी":"All"} ({SCHEME_DB.length})
          </div>
          {cats.map(cat=>(
            <div key={cat.filterKey} onClick={()=>setFilter(cat.filterKey)}
...
export default function YojanaSetu(){
  ...
  return(
    <div style={{fontFamily:bf,background:"#f5f5f0",maxWidth:420,margin:"0 auto",position:"relative",height:"100vh",display:"flex",flexDirection:"column",overflowX:"hidden",boxShadow:"0 0 60px rgba(0,0,0,0.15)",opacity:langAnim?0.7:1,transition:"opacity 0.12s"}}>
      <style>{`
      ...
      {activeTab==="home"&&(
        <div style={{flex:1,overflowY:"auto"}}>
            ...
            <div style={{padding:"0 20px"}}>
              <div className={`sb ${searchFocused?"fc":""}`}
                ...
                <input value={searchText} onChange={e=>setSearchText(e.target.value)}
                  ...
                  onBlur={()=>{setSearchFocused(false);if(searchText.trim())setActiveTab("search");}}
          ...
          <div className={`fu s1 ${loaded?"show":""}`}
            ...
            {t.stats.map((s,i)=>(
              <div key={i} style={{flex:1,textAlign:"center",borderRight:i<2?"1px solid #f0f0f0":"none",padding:"0 8px"}}>
                <div style={{fontSize:20,fontWeight:800,color:i===0?"#FF9933":i===1?"#003580":"#138808"}}>{s.number}</div>
          ...
          <div style={{padding:"14px 16px 100px"}}>
            ...
            <div className={`fu s2 ${loaded?"show":""}`} style={{marginBottom:16}}>
            ...
            <div className={`fu s4 ${loaded?"show":""}`}
              ...
              <div onClick={()=>window.open("tel:1800111555")}
      ...
      {/* ── AI & PROFILE — coming soon ── */}
      {["ai","profile"].includes(activeTab)&&(
      ...
      {/* Bottom nav */}
      <div style={{background:"#fff",borderTop:"1.5px solid #f0f0f0",padding:"8px 4px 20px",display:"flex",boxShadow:"0 -4px 20px rgba(0,0,0,0.07)",zIndex:100,flexShrink:0}}>
        {navItems.map(item=>(
          <div key={item.tab} className="bn" onClick={()=>setActiveTab(item.tab)} style={{opacity:activeTab===item.tab?1:0.45}}>
            <div style={{fontSize:20}}>{item.icon}</div>
            <div style={{fontSize:9,fontWeight:activeTab===item.tab?700:500,color:activeTab===item.tab?"#003580":"#888",fontFamily:bf}}>{item.label}</div>
            {activeTab===item.tab&&<div style={{width:4,height:4,borderRadius:"50%",background:"#FF9933"}}/>}
          </div>
        ))}
      </div>
