...
function SchemesTab({lang,dark=false}){
  ...
  const filtered=useMemo(()=>filter==="all"
    ? SCHEME_DB
    : getSchemesForCategory(filter),[filter]);

  const national=useMemo(()=>filtered.filter(s=>s.scope==="national"),[filtered]);
  const state=useMemo(()=>filtered.filter(s=>s.scope==="state"),[filtered]);

  return(
    <div style={{flex:1,display:"flex",flexDirection:"column",overflowY:"auto",background:th.appBg}}>
      {/* Header */}
      <div style={{background:th.card,padding:"16px 16px 0",position:"sticky",top:0,zIndex:10,borderBottom:`1px solid ${th.border}`}}>
        <div style={{fontSize:17,fontWeight:800,color:th.text,fontFamily:bf,marginBottom:10}}>{t.allSchemes||"All Schemes"}</div>
        {/* Filter pills */}
        <div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:12}}>
          <div onClick={()=>{setFilter("all");setExpandedId(null);}} style={{flexShrink:0,padding:"6px 14px",borderRadius:20,fontSize:12,fontWeight:700,background:filter==="all"?"#003580":th.pillBg,color:filter==="all"?"#fff":th.textMid,cursor:"pointer",border:`1.5px solid ${filter==="all"?"#003580":th.border2}`}}>
...
function ProfileTab({lang,profile,setProfile,toggleLang,onViewChecker,dark=false,toggleDark}){
  ...
  const handleVerify=()=>{
    ...
    // Pre-fill step 2 from eligibility if available
    if(savedAnswers){
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",background:th.appBg,overflowY:"auto"}}>
      ...
      <Card dark={dark}>
        ...
        <div onClick={handleGetOtp}
          style={{background:phone.length===10?"linear-gradient(135deg,#FF9933,#FF8C00)":"#ddd",borderRadius:14,padding:"15px",textAlign:"center",fontSize:15,fontWeight:700,color:"#fff",cursor:phone.length===10?"pointer":"default",fontFamily:bf,boxShadow:phone.length===10?"0 6px 22px rgba(255,153,51,0.42)":"none",transition:"all 0.22s"}}>
          {pt.getOtpBtn}
        </div>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",background:th.appBg,overflowY:"auto"}}>
        ...
        <Card dark={dark}>
          ...
          <div onClick={handleVerify}
            style={{background:otpFull?"linear-gradient(135deg,#003580,#1a56db)":"#ddd",borderRadius:14,padding:15,textAlign:"center",fontSize:15,fontWeight:700,color:"#fff",cursor:otpFull?"pointer":"default",fontFamily:bf,boxShadow:otpFull?"0 6px 22px rgba(0,53,128,0.36)":"none",transition:"all 0.22s",marginBottom:16}}>
            {pt.verifyBtn}
          </div>
          ...
          <div style={{marginTop:14,textAlign:"center",fontSize:10.5,color:"#ccc",fontFamily:bf}}>{pt.demoNote}</div>
        </Card>
      </div>
    );
  }
...
export default function YojanaSetu(){
  ...
  return(
    <div className="app-root" style={{fontFamily:bf,background:th.appBg,maxWidth:420,margin:"0 auto",position:"relative",display:"flex",flexDirection:"column",overflowX:"hidden",boxShadow:"0 0 60px rgba(0,0,0,0.15)",opacity:langAnim?0.7:1,transition:"opacity 0.12s,background 0.3s"}}>
      ...
      {activeTab==="ai"&&(
        <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",padding:"32px 28px",background:th.appBg}}>
          ...
          <div style={{fontSize:20,fontWeight:800,color:th.text,fontFamily:bf,textAlign:"center",marginBottom:8}}>{isHindi?"AI सहायक आ रहा है":"AI Assistant Coming Soon"}</div>
          <div style={{fontSize:13,color:th.textSub,textAlign:"center",lineHeight:1.65,fontFamily:bf,marginBottom:24}}>
            {isHindi
          ...
          <div style={{width:"100%",maxWidth:300,display:"flex",flexDirection:"column",gap:10}}>
            {(isHindi?["📋 किसी भी योजना को समझाएं","✅ मेरी पात्रता बताएं","📝 आवेदन में मदद करें","🔍 मेरे लिए योजना खोजें"]:["📋 Explain any scheme in simple terms","✅ Tell me if I qualify","📝 Walk me through applying","🔍 Find schemes just for me"]).map((f,i)=>(
              <div key={i} style={{background:th.card,borderRadius:12,padding:"11px 14px",fontSize:13,color:th.text,fontFamily:bf,border:`1.5px solid ${th.border2}`,boxShadow:dark?"0 1px 4px rgba(0,0,0,0.2)":"0 1px 4px rgba(0,0,0,0.04)"}}>{f}</div>
            ))}
          </div>
